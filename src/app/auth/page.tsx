...
export default function AuthPage() {
  ...
  const onSubmit = async (values:AuthFormValues) => {
    ...
    try {
      if (mode === "register") {
        await authService.registerWithEmail(values.email, values.password);
      } else {
            ...
            <CardContent className="px-6 sm:px-8 py-1">
              <form onSubmit={form.handleSubmit(onSubmit)} className="mt-6">
                <FieldGroup className="gap-2.5">
                          ...
                          <Field data-invalid={fieldState.invalid}>
                            ...
                            <Container as={"div"} className="relative">
                              <Input
                                  ...
                                  disabled={isBusy}
                                  aria-invalid={fieldState.invalid}
                                  className="h-auto rounded-[9px] border-[#2a2a2e] bg-[#18181b] px-3.5 pr-10 py-[9px] text-[13px] text-[#e8e8ea] transition-colors duration-150 focus-visible:border-[#e8493f]/50 focus-visible:ring-[#e8493f]/20 aria-invalid:border-[#e8493f]/60 [&:-webkit-autofill]:[-webkit-text-fill-color:#e8e8ea] [&:-webkit-autofill]:[-webkit-box-shadow:0_0_0px_1000px_#18181b_inset] [&:-webkit-autofill]:[transition:background-color_9999s_ease-in-out_0s]"
                              />
                              <button
                                  type="button"
                  ...
                  <Typography variant="p" className="text-center text-[13px] text-[#a1a1aa]">
                    {mode === "login" ? "Don't have an account? " : "Already have an account? "}
                    <button
                        type="button"
                        onClick={switchMode}
                        disabled={isBusy}
                        className="cursor-pointer font-medium text-[#e8493f] underline underline-offset-2 disabled:opacity-60"
                    >
