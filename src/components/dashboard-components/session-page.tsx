...
import type {SongData, SongRecommendation} from "@/type/song-recommendation";
...
function toDisplaySession(song: SongRecommendation): SongData {
    return {
        id: song.audioUrl,
        title: song.title,
        artist: song.artist,
        ...
        matchedEffect: "matchedEffect" in song ? song.matchedEffect : undefined,
        relatedSymptoms: "relatedSymptoms" in song ? song.relatedSymptoms : undefined,
    };
}
...
export default function SessionsPage() {
    ...
    const selectedType: SearchType =
        typeParam === "mood" || typeParam === "condition"
            ? typeParam
            : null;

    const selectedValue = selectedType ? valueParam : null;
    ...
    const displaySessions = useMemo(
        () => hasFilter ? songs.map(toDisplaySession) : [],
        [songs, hasFilter]
    );
